 ...
 * @author jonathan
 */
public class KeyedIndexInvertedIndex<BM> implements MiruInvertedIndex<BM> {
    ...
    public KeyedIndexInvertedIndex(MiruBitmaps<BM> bitmaps,
    ...
    public int lastId() throws Exception {
        if (lastId == Integer.MIN_VALUE) {
            synchronized (mutationLock) {
                byte[] lastIdBytes = keyedIndex.get(indexKey.keyBytes);
                if (lastIdBytes != null) {
                    lastId = FilerIO.bytesInt(lastIdBytes);
                } else {
                    lastId = -1;
                }
            }
