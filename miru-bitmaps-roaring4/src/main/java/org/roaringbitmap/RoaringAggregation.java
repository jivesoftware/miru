...
public class RoaringAggregation {
    ...
    public static void andNot(final RoaringBitmap answer,
        ...
        main:
        if (pos1 < length1 && pos2 < length2) {
            short s1 = x1.highLowContainer.array[pos1].key;
            short s2 = x2.highLowContainer.array[pos2].key;
            do {
                if (s1 < s2) {
                    ...
                    s1 = x1.highLowContainer.array[pos1].key;
                } else if (s1 > s2) {
                    pos2++;
                    if (pos2 == length2) {
                        break main;
                    }
                    s2 = x2.highLowContainer.array[pos2].key;
                } else {
                    final Container c = x1.highLowContainer.array[pos1].value.andNot(x2.highLowContainer.array[pos2].value);
                    if (c.getCardinality() > 0) {
                        answer.highLowContainer.append(s1, c);
                    }
                    pos1++;
