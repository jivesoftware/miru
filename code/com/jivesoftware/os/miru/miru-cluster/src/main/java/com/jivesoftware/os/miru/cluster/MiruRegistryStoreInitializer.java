 ...
 *
 * - e: Tenant1 = void - e: Tenant2 = void - e: Tenant3 = void
 *
 * miru.reg.th [Tenant, Host] [e=expectedTenantPartitions]
 *
 * - e: Partition1 = void - e: Partition2 = void - e: Partition3 = void
 *
 * miru.reg.tp [Tenant, Partition] [r=replica]
 *
 * TODO could be - r: Host1 = 1 (timestamp for recency) - r: InverseDateAdded1 = Host1 - r: InverseDateAdded2 = Host2 - r: InverseDateAdded3 = Host3
 *
 * miru.reg.t [Void, Tenant] [t=topology, c=config, p=partition]
 *
 * TODO store sip time - t: Partition1 Host1 = State BackingStorage SipTimestamp (timestamp for staleness) - t: Partition1 Host1 = State BackingStorage
 * (timestamp for staleness) - t: Partition1 Host2 = State BackingStorage (timestamp for staleness) - t: Partition1 Host3 = State BackingStorage (timestamp for
 * staleness) - t: Partition2 Host1 = State BackingStorage (timestamp for staleness) - t: Partition2 Host2 = State BackingStorage (timestamp for staleness) - t:
 * Partition2 Host3 = State BackingStorage (timestamp for staleness)
 *
 * - c: NumberOfReplicas = 3 - c: TopologyIsStaleAfterMillis = 1_000_000
 *
...
public class MiruRegistryStoreInitializer {
    ...
    public MiruRegistryStore initialize(String tableNameSpace,
            SetOfSortedMapsImplInitializer<? extends Exception> setOfSortedMapsImplInitializer) throws Exception {
        ...
        return new MiruRegistryStore(hostsRegistry, expectedTenantsRegistry, expectedTenantPartitionsRegistry, replicaRegistry, topologyRegistry, configRegistry, writerPartitionRegistry, activityLookupTable);
    }
