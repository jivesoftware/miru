...
public class MiruBitmapsTest {
    ...
    public <BM> void testAndNotWithCardinalityAndLastSetBit(MiruBitmaps<BM> bitmaps) throws Exception {
        ...
        MiruBitmaps.CardinalityAndLastSetBit cardinalityAndLastSetBit = bitmaps.andNotWithCardinalityAndLastSetBit(container, original, not);
        for (int i = 0; i < numBits; i++) {
            if (i < notBits) {
                assertFalse(bitmaps.isSet(container, i * 137));
            } else {
    ...
    public <BM> void testAndWithCardinalityAndLastSetBit(MiruBitmaps<BM> bitmaps) throws Exception {
        ...
        MiruBitmaps.CardinalityAndLastSetBit cardinalityAndLastSetBit = bitmaps.andWithCardinalityAndLastSetBit(container, ands);
        for (int i = 0; i < numBits; i++) {
            if (i < (numBits - andBits)) {
                assertFalse(bitmaps.isSet(container, i * 137));
            } else {
