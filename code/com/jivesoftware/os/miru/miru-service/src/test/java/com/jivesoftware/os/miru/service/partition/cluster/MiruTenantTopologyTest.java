...
public class MiruTenantTopologyTest {
    ...
    public void testCheckForPartitionAlignment() throws Exception {
        ...
        tenantTopology.checkForPartitionAlignment(Lists.newArrayList(
            new MiruPartitionActiveUpdate(tenantId, p0.getId(), true, Long.MAX_VALUE, Long.MAX_VALUE)));
        ...
        tenantTopology.checkForPartitionAlignment(Lists.newArrayList(
            new MiruPartitionActiveUpdate(tenantId, p0.getId(), true, Long.MAX_VALUE, Long.MAX_VALUE),
            new MiruPartitionActiveUpdate(tenantId, p1.getId(), true, Long.MAX_VALUE, Long.MAX_VALUE)));

        verify(localPartitionFactory).create(same(bitmaps), eq(new MiruPartitionCoord(tenantId, p1, localhost)), anyLong());
        verifyNoMoreInteractions(localPartitionFactory);

        tenantTopology.checkForPartitionAlignment(Lists.newArrayList(
            new MiruPartitionActiveUpdate(tenantId, p0.getId(), false, Long.MAX_VALUE, Long.MAX_VALUE),
            new MiruPartitionActiveUpdate(tenantId, p1.getId(), false, Long.MAX_VALUE, Long.MAX_VALUE)));
