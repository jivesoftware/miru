...
public class MiruHybridField<BM> implements MiruField<BM>, BulkExport<Iterator<BulkEntry<MiruTermId, MiruFieldIndexKey>>> {
    ...
    public MiruHybridField(MiruFieldDefinition fieldDefinition, MiruInMemoryIndex<BM> index, ByteBufferFactory byteBufferFactory, int initialPageCapacity)
        throws Exception {
        ...
        for (int i = 0; i < KEY_SIZE_THRESHOLDS.length; i++) {
            ...
            mapStoreBuilder.add(keySize,
                new ByteBufferFactoryBackedMapChunkFactory(keySize, true, PAYLOAD_SIZE, false, initialPageCapacity, byteBufferFactory),
