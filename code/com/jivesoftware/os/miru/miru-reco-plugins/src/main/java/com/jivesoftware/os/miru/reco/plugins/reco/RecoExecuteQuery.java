...
public class RecoExecuteQuery implements ExecuteQuery<RecoResult, RecoReport> {
    ...
    @Override
    public <BM> RecoResult executeLocal(MiruQueryHandle<BM> handle, Optional<RecoReport> report) throws Exception {
    ...
    public RecoResult executeRemote(RequestHelper requestHelper, MiruPartitionId partitionId, Optional<RecoResult> lastResult) throws Exception {
        return new RecoRemotePartitionReader(requestHelper).collaborativeFilteringRecommendations(partitionId, query, lastResult);
    }
