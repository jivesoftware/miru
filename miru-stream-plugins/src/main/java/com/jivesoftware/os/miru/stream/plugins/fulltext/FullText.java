...
public class FullText {
    ...
    public <BM> FullTextAnswer getActivityScores(MiruBitmaps<BM> bitmaps,
        ...
        if (request.query.strategy == FullTextQuery.Strategy.TF_IDF) {
            activityScores = collectTfIdf(bitmaps, requestContext, request, lastReport, answer, termCollector);
        } else if (request.query.strategy == FullTextQuery.Strategy.TIME) {
            activityScores = collectTime(bitmaps, requestContext, request, lastReport, answer);
        } else {
    ...
    private <BM> List<ActivityScore> collectTfIdf(MiruBitmaps<BM> bitmaps,
        ...
        while (iter.hasNext()) {
            ...
            if (i == batchSize) {
                batchTfIdf(requestContext, request, internExtern, primaryFieldIndex, termMultipliers, scored, minScore, acceptableBelowMin, ids);
                i = 0;
            }
        }
    ...
    private <BM> void batchTfIdf(MiruRequestContext<BM, ?> requestContext,
        ...
        for (Map.Entry<FieldAndTermId, Float> entry : termMultipliers.entrySet()) {
            ...
            long[] tf = primaryFieldIndex.getCardinalities(fieldAndTermId.fieldId, fieldAndTermId.termId, ids);
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            if (scores[i] > minScore) {
                RawBitScore bitScore = new RawBitScore(new Promise<>(() -> {
                    MiruInternalActivity internalActivity = requestContext.getActivityIndex().get(request.tenantId, ids[_i]);
    ...
    private <BM> List<ActivityScore> collectTime(MiruBitmaps<BM> bitmaps,
        ...
        while (iter.hasNext()) {
            int lastSetBit = iter.next();
            MiruInternalActivity internalActivity = requestContext.getActivityIndex().get(request.tenantId, lastSetBit);
            float score = 0f; //TODO ?
